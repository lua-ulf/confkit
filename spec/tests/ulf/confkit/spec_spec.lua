	...
	describe("field", function()
		...
		describe("parse", function()
			...
			it("parses a field with only description and type", function()
				...
				assert.equal(Constants.FIELD_BEHAVIOUR.OPTIONAL_FIELD, f.behaviour)
				assert.equal("A test description", f.description)
				assert.equal("number", f.type)
			end)

			it("returns field.behaviour=NON_FIELD if no type is provided for fields without a value", function()
				local f = spec.field.parse("test_key", { "A description for an optional field" })
				assert.equal(Constants.FIELD_BEHAVIOUR.NON_FIELD, f.behaviour)
			end)

			it("returns field.behaviour=NON_FIELD if description as the last list element is not a string", function()
			...
			describe("using hooks", function()
				it("parses a field if hook is set and a function", function()
					...
					assert.equal(Constants.FIELD_BEHAVIOUR.MANDATORY_FIELD, f.behaviour)
